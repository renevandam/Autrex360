...
export default async function handler(req, res) {
  ...
  if (action === 'deactivate') {
    ...
    // Ban for ~100 years - effectively permanent, but reversible via 'reactivate'.
    // This blocks the account at the Supabase Auth level itself (no session, no
    // password reset, nothing) rather than relying on the app to check a flag everywhere.
